...
function buildServicePayload(body) {
  return {
    ...
    process:
      Array.isArray(body.process) && body.process.length
        ? body.process
        : ["Discover", "Design", "Build", "Launch"],
    packages:
      Array.isArray(body.packages) && body.packages.length
        ? body.packages
        : [
            {
              name: "Custom",
              price: "Custom",
              features: ["Scope on request"],
            },
          ],
    results:
...
function serviceQuery(idOrSlug) {
  return /^[a-f\d]{24}$/i.test(idOrSlug)
    ? { _id: idOrSlug }
    : { slug: idOrSlug };
}
...
router.put("/:id", verifyToken, requireAdmin, async (req, res, next) => {
  try {
    const service = await Service.findOneAndUpdate(
      serviceQuery(req.params.id),
      buildServicePayload(req.body),
      {
        new: true,
        runValidators: true,
      }
